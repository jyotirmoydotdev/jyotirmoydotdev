...
export default function Page() {
  ...
  React.useEffect(() => {
    if (getQ) {
      const q = getQ[Math.floor(Math.random() * getQ.length)];
      setRandomQuestion(q.titleSlug + "/" + q.id);
    }
  }, []);

  if (!getQ) {
    return (
      <div className="h-full flex-1 flex-col space-y-8 px-4 sm:px-8 sm:py-4 md:flex max-w-6xl">
        ...
        <div className="grid mb-4">
          <div className="flex gap-2 overflow-x-scroll no-scrollbar">
            {alltags.map((tag, i) => (
              <div
                key={i}
                className=" bg-muted/50 border text-xs sm:text-sm text-nowrap px-4 py-1.5 rounded-lg text-muted-foreground"
              >
                {tag.label}
              </div>
            ))}
