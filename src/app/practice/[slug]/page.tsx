...
export default async function Page({
  ...
  const { default: Practice } = await import(`@/markdown/${slug}.mdx`);
  ...
  const practiceindex = practices.findIndex(
    (practice) => practice.titleSlug === slug,
  );
  const practice = practices[practiceindex];
  const moredata = [
    ...practices.slice((practiceindex + 1) % practices.length),
...
export function generateStaticParams() {
  return practices.flatMap((practice) => {
    return { slug: practice.titleSlug };
  });
}
