...
const Content = ({
  ...
  practiceDataIndex: number;
  practiceContent: (string | string[])[];
}) => {
    ...
    >
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <div className="pb-[10rem] pl-3 h-[calc(100vh-20rem)] no-scrollbar overflow-scroll">
              <ContentComp contents={practiceContent} />
            </div>
          </SidebarGroupContent>
        </SidebarGroup>
      </SidebarContent>
      <SidebarFooter className=" fixed -bottom-2">
        ...
        <SidebarMenu className=" px-4 pt-4 pb-2 whitespace-nowrap transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border border-input hover:text-accent-foreground  justify-start rounded-t-[0.5rem] bg-sidebar text-sm shadow-none">
          <div className=" font-bold text-orange-400">More Practice</div>
          <Separator />
          {Array(2)
            .fill(null)
            .map((_, i) => {
              const post = practices.at(
                (practiceDataIndex + i + 1) % practices.length,
              )!;
              return (
                <Link
                  key={i}
                  href={post?.titleSlug}
                  className="w-[260px] text-wrap flex flex-col gap-2 py-2 group/more hover:bg-gray-100/10 px-2 transition-all rounded-md mt-2"
                >
                  <div className=" font-black group-hover/more:text-orange-400 line-clamp-1 text-sm ">
                    {post.id}
                    {". "}
                    {post.title} -{" "}
