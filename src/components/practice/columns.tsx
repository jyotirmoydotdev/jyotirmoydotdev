...
export const columns: ColumnDef<practiceType>[] = [
  {
    ...
    cell: ({ row }) => {
      return <span className="pl-4 ">{row.original.exid}</span>;
    },
  },
  {
    accessorKey: "title",
            ...
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                <TooltipTrigger className="flex gap-2">
                  <span className="rounded-sm bg-muted line-clamp-1 px-1.5 py-0.5">
                    {tags[0]}
                  </span>
                  {tags.length > 1 && (
                ...
                <TooltipContent className="bg-muted backdrop-blur-sm flex gap-2 px-2 py-2 text-black dark:text-white">
                  {tags.map((tag, i) => (
                    <span key={i} className="rounded-sm border px-1.5 py-0.5">
                      {tag}
                    </span>
                  ))}
                </TooltipContent>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span
          className={`${difficulty === "Medium" ? "text-yellow-500" : difficulty === "Easy" ? "text-green-500" : "text-red-500"}`}
        >
          {difficulty}
        </span>
      );
  ...
  },
  {
    ...
    header: ({ column }) => <ColumnHeader column={column} title="Solution" />,
    cell: ({ row }) => {
      ...
      if (hasSol) {
        return (
          <div className="flex gap-2">
            ...
            {row.original.hasVideoSolution && (
              <FileVideo className="text-[#b260ea] size-5" />
            )}
          </div>
        );
      } else {
        return <FileX className="text-gray-400 size-5" />;
      }
    },
  },
  {
