---
title: "Efficiently Counting Bad Pairs in an Array"
date: "2025-02-09"
---
...
Notice that the condition for a pair **not** to be bad (i.e., a **good pair**) is:

\[
\text{nums}[j] - \text{nums}[i] = j - i
\]
...
{/* \[
\text{bad\_pairs} = i - \text{hash}[diff]
\] */}
...
Below is the Go function that implements this logic:

```go
func countBadPairs(nums []int) int64 {
    // Map to track the frequency of each difference (nums[i] - i)
    hash := make(map[int]int)
    counter := 0
    
    // Iterate over the array, using each index to form pairs with previous indices
    for i, num := range nums {
        diff := num - i
        // Total pairs with previous indices is 'i'
        // Subtract the count of good pairs (hash[diff]) to get bad pairs
        counter += i - hash[diff]
        // Update the frequency map for this difference
        hash[diff]++
    }
...
By transforming the condition for a good pair into a relationship based on the difference **nums[i] - i**, we can avoid checking every pair individually. This not only simplifies the logic but also significantly improves the performance of the solution.

Feel free to leave any questions or comments below if you'd like to discuss this approach further or explore additional optimizations!
